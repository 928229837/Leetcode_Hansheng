...
#  [L, R] (inclusive) having a prime number of set bits in their
...
def countPrimeSetBits(L, R):
    ...
    for num in range(L, R + 1):
        ...
        if temp in primList:
            total += 1
        elif temp > 19 and temp % 2 != 0 and all(
                temp % i != 0 for i in range(3, int(math.sqrt(temp)) + 1, 2)) and temp > 1:
            total += 1
            primList.update({temp})
    return total
